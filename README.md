...
Board được cập nhật bởi **monitor chạy trên VM riêng** (systemd timer, mỗi 15 phút):
trong giờ GD làm tươi equity, sau đóng cửa chạy quyết định 1 lần/ngày, luôn có
"nhịp tim" mỗi ngày. Xem `scripts/monitor.py`. (GitHub Actions cron đã tắt để tránh
push trùng; chỉ giữ `workflow_dispatch` chạy tay dự phòng.)
